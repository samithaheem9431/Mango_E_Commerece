...
function buildNewOrderAdminHTML({ orderId, items, subtotal, deliveryFee, grandTotal, shippingAddress, customerName, customerEmail, riderNotes }) {
  const trackingId = String(orderId).toUpperCase();
  ...
  const itemRows = items.map((item) => `
    <tr>
      <td style="padding:12px 16px;border-bottom:1px solid #f0fdf4;">
        <span style="font-size:14px;color:#14532d;font-weight:600;">${item.name}</span>
        <span style="display:inline-block;background:#fde047;color:#0b1a0b;font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;margin-left:6px;vertical-align:middle;">${item.boxSize || ""}</span>
      </td>
      <td style="padding:12px 16px;border-bottom:1px solid #f0fdf4;text-align:center;color:#166534;font-size:14px;">${item.quantity}</td>
      <td style="padding:12px 16px;border-bottom:1px solid #f0fdf4;text-align:right;color:#166534;font-size:14px;">Rs ${item.price.toLocaleString()}</td>
      <td style="padding:12px 16px;border-bottom:1px solid #f0fdf4;text-align:right;font-weight:700;color:#d97706;font-size:14px;">Rs ${(item.price * item.quantity).toLocaleString()}</td>
  ...
  const customItemTable = `
  <tr>
    <td style="padding:24px 40px 0;">
      <h3 style="margin:0 0 12px;font-size:15px;font-weight:700;color:#14532d;text-transform:uppercase;letter-spacing:1px;">🛒 Order Items</h3>
      <table width="100%" cellpadding="0" cellspacing="0" style="border:1.5px solid #d1fae5;border-radius:12px;overflow:hidden;">
        <tr style="background:#15803d;">
          <th style="padding:11px 16px;text-align:left;color:#fff;font-size:12px;font-weight:700;text-transform:uppercase;">Product</th>
          <th style="padding:11px 16px;text-align:center;color:#fff;font-size:12px;font-weight:700;text-transform:uppercase;">Qty</th>
  ...
  const rows = `
    ${emailHeader("🔔", {
      ...
      headingColor: "#14532d", textColor: "#166534"
    }, `You have received a new order on <strong>Aam-e-Khaas</strong>.<br/>Review the details below and manage it in the Admin Dashboard.`)}
    ...
    <tr>
      <td style="padding:28px 40px 0;">
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0fdf4;border:2px dashed #86efac;border-radius:14px;">
          <tr>
            <td style="padding:18px 24px;">
              <p style="margin:0 0 4px;font-size:11px;color:#16a34a;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;">Order ID</p>
              <p style="margin:0;font-size:17px;font-weight:800;color:#14532d;letter-spacing:1px;word-break:break-all;">#${trackingId}</p>
            </td>
            <td style="padding:18px 24px;text-align:right;">
              <span style="display:inline-block;background:#dcfce7;color:#15803d;font-size:12px;font-weight:700;padding:6px 14px;border-radius:999px;border:1.5px solid #86efac;">🆕 New Order</span>
            </td>
          </tr>
        </table>
      </td>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#fff7ed;border:1.5px solid #fed7aa;border-radius:12px;">
          <tr>
            <td style="padding:16px 20px;">
              ...
              <p style="margin:0;font-size:14px;color:#7c2d12;font-weight:600;"><span style="color:#9a3412;font-weight:normal;">Name:</span> ${customerName}</p>
              <p style="margin:4px 0 0;font-size:13px;color:#7c2d12;font-weight:600;"><span style="color:#9a3412;font-weight:normal;">Email:</span> ${customerEmail}</p>
              <p style="margin:4px 0 0;font-size:13px;color:#7c2d12;font-weight:600;"><span style="color:#9a3412;font-weight:normal;">Phone:</span> ${shippingAddress.phone}</p>
              <p style="margin:4px 0 0;font-size:13px;color:#7c2d12;font-weight:600;"><span style="color:#9a3412;font-weight:normal;">Address:</span> ${shippingAddress.address}, ${shippingAddress.city}</p>
              ${riderNotes ? `<p style="margin:8px 0 0;font-size:12px;color:#9a3412;background:#ffedd5;padding:8px;border-radius:6px;font-style:italic;"><strong>Rider Notes:</strong> ${riderNotes}</p>` : ""}
            </td>
          </tr>
        </table>
      </td>
...
async function sendNewOrderAdminNotification({ orderId, items, subtotal, deliveryFee, grandTotal, shippingAddress, customerName, customerEmail, riderNotes }) {
  if (!guardEnv()) return;
  const resend = getResend();
  const adminReceiverEmail = process.env.ORDER_RECEIVER_EMAIL || "ammekhaas@gmail.com";
  const { error } = await resend.emails.send({
    from: `Aam-e-Khaas <${process.env.RESEND_FROM_EMAIL}>`,
    to: adminReceiverEmail,
    subject: `🔔 New Order Received - #${String(orderId).toUpperCase()}`,
    html: buildNewOrderAdminHTML({ orderId, items, subtotal, deliveryFee, grandTotal, shippingAddress, customerName, customerEmail, riderNotes })
  });
  if (error) throw new Error(error.message);
  console.log(`[Email] New order notification sent to ${adminReceiverEmail}`);
}
...
module.exports = { sendOrderConfirmed, sendOrderCancelled, sendOrderDelivered, sendNewOrderAdminNotification, testEmailConnection };
